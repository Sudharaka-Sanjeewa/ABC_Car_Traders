    ...
    public partial class Cars : Form
    {
        ...
        public void FilldgvCars(string Search)
        {
            try
            {
                ...
                MySqlDataAdapter adp = new MySqlDataAdapter("SELECT * FROM car WHERE CONCAT(Model, Price)LIKE'%" + Search + "%'", con);
                DataTable tb = new DataTable();
                adp.Fill(tb);
                dgvCars.DataSource = tb;

                DataGridViewTextBoxColumn AddDate = new DataGridViewTextBoxColumn();
                AddDate = (DataGridViewTextBoxColumn)dgvCars.Columns[3];
        ...
        private void dgvCars_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            lblModel.Text = dgvCars.CurrentRow.Cells[1].Value.ToString();
            lblPrice.Text = dgvCars.CurrentRow.Cells[2].Value.ToString();
